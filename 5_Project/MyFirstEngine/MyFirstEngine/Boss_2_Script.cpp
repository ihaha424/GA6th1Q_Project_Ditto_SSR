...
Boss_2_Script::Boss_2_Script(GameObject* pOwner, std::string ObjName)
	:ScriptComponent(pOwner, ObjName), m_cCollider(nullptr), m_cRender(nullptr)
{
	Start();
}

void Boss_2_Script::AnimationInitialize()
{
	//애니메이션
	...
	m_cTexture->m_tTexture[0] = ResourceManager::GetInstance()->LoadTexture(L"BossTexture1", L"\\Animation\\Player\\boss_002_001.bmp");
	m_cTexture->m_tTexture[1] = ResourceManager::GetInstance()->LoadTexture(L"BossTexture2", L"\\Animation\\Player\\boss_002_002.bmp");
	m_cTexture->m_tTexture[2] = ResourceManager::GetInstance()->LoadTexture(L"BossTexture3", L"\\Animation\\Player\\boss_002_003.bmp");
	m_cTexture->m_tTexture[3] = ResourceManager::GetInstance()->LoadTexture(L"BossTexture4", L"\\Animation\\Player\\boss_002_004.bmp");
	m_cTexture->m_tTexture[4] = ResourceManager::GetInstance()->LoadTexture(L"BossTexture5", L"\\Animation\\Player\\boss_002_005.bmp");
	m_cTexture->m_uExcep = RGB(255, 0, 255);

}
void Boss_2_Script::AnimationUpdate(float fixedDeltaTime)
{
...
void Boss_2_Script::FixedUpdate(float fixedDeltaTime)
{
...
void Boss_2_Script::Pattern101(int x, int y)
{
...
void Boss_2_Script::Pattern102(int x, int y)
{
	for (int i = 0; i < 5; ++i)
	{
		for (int j = 0; j < 5; ++j)
...
void Boss_2_Script::Pattern103(int x, int y)
{
...
void Boss_2_Script::Pattern401(float fixedDeltaTime)
{
...
void Boss_2_Script::Pattern402(float fixedDeltaTime)
{
